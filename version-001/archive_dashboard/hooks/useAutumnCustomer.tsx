...
// Temporary placeholder for useAutumnCustomer while Autumn is disabled
function useAutumnCustomer(params?: { skip?: boolean }) {
	return {
		customer: null,
		refetch: async () => ({ customer: null }),
		loading: false,
		error: null,
	};
}

// Type definition for useCustomer params
...
const AutumnCustomerContext = createContext<AutumnCustomerContextType | null>(
	null
);
...
export function useRefreshCustomer() {
	...
	if (!context) {
		...
		return async () => {
			console.warn(
				"useRefreshCustomer called outside of AutumnCustomerProvider"
			);
		};
	}
