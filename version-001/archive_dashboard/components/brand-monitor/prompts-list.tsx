...
import { AIProvider, BrandPrompt } from "@/lib/types";
...
export function PromptsList({
	...
	const renderStatusIcon = (status: string) => {
		switch (status) {
			case "running":
				return <Loader2 className="h-4 w-4 animate-spin text-light-blue" />;
			case "completed":
				return <Check className="h-4 w-4 text-light-orange" />;
			case "failed":
				return <X className="h-4 w-4 text-gray-500" />;
			default:
				return <div className="h-4 w-4 rounded-full bg-white-200" />;
		}
	};

	return (
		<Card className="p-2 bg-card text-card-foreground gap-6 rounded-xl border py-6 shadow-sm border-white-200 h-full flex flex-col">
			<CardHeader>
			...
			<CardContent className="space-y-4">
				{/* Add prompt input */}
				{showAddPrompt && (
					<div className="flex gap-2 p-4 bg-white-50 rounded-lg">
						<Input
				...
				<div className="space-y-2">
					{prompts.length === 0 && !isAnalyzing ? (
						...
						prompts.map((prompt) => (
							<div
								key={prompt.id}
								className="flex items-center gap-3 p-4 bg-white-50 rounded-lg hover:bg-white-100 transition-colors"
							>
								{/* Prompt text */}
				...
				{availableProviders.length > 0 && (
					<div className="pt-4 border-t">
						<p className="text-xs text-gray-600 mb-2">Available providers:</p>
