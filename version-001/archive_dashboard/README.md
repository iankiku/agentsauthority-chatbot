...
Get your SaaS running in minutes with authentication, billing, AI chat, and
brand monitoring. Zero-config setup with Next.js 15, TypeScript, and PostgreSQL.
...
If you prefer to run the setup commands individually or need more control over
...
| **Web Scraping**   | Placeholder (Firecrawl removed)              |
...
- Copy the connection string (use "Transaction" mode) to `DATABASE_URL` in
  `.env.local`
...
- **Firecrawl**: Removed from project
- **OpenAI**:
...
# Brand Monitor
# FIRECRAWL_API_KEY=         # Removed from project
...
- **Auth Issues**: Ensure `BETTER_AUTH_SECRET` is set and matches between
  deploys
